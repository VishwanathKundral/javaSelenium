...
import com.sdet.auto.TestHelper.InOutLibrary;
...
public class WebDriverExtensions {
    ...
    public static void seleniumExceptionHandler(Exception ex) {
        ...
        InOutLibrary.writeLine(String.format("WebDriver Exception Handler Caught Exception: [%s]", exceptionName));
        screenshot();
        InOutLibrary.writelineEnd();
    }

    public static void screenshot() {
        File screenshot;
        String testName = String.format(InOutLibrary.getTestName() + "_" + InOutLibrary.getUniqueIdentifier() + ".Png");

        try {
            System.out.println("Attempting Selenium Screenshot  ...");
            Thread.sleep(1000);
            screenshot = ((TakesScreenshot) driver).getScreenshotAs(OutputType.FILE);
            String screenshotDir = InOutLibrary.getDirPath() + "/target/outputs/screenshots/";
            File dir = new File(screenshotDir);

            if (!dir.exists()) {
                InOutLibrary.writeLine("No Folder Found");
                dir.mkdirs();
